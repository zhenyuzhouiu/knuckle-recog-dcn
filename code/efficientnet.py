...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    This function is taken from the rwightman. sz
    It can be seen here:
    ...
    # (1,)*(x.ndim-1) repeate (1,) (x.ndim-1) times -> (1,) * 3 = (1,1,1)
    # (1,) + (1,) = (1, 1)
    # so shape -> [batch_size, 1, 1, 1}
    shape = (x.shape[0],) + (1,) * (x.ndim - 1)  # work with diff dim tensors, not just 2D ConvNets
    random_tensor = keep_prob + torch.rand(shape, dtype=x.dtype, device=x.device)
    random_tensor.floor_()  # binarize floor_() will directly change the tensor & floor()
    # training process: x/keep_p -> keep_p * x/ keep_p = x
    # testing process: it will not have any process for the output x
...
class ConvBNActivation(nn.Sequential):
    def __init__(self,
        ...
        # group represents whether you use the depth-wise convolution
        # when your convolution use batch normalization, you should not to add bias
...
# using convolution to replace fully connection layer
class SqueezeExcitation(nn.Module):
    def __init__(self,
                 input_c: int,   # block input channel
                 expand_c: int,  # block expand channel, just after MBConv's first Conv 1x1 s1
...
class InvertedResidualConfig:
    ...
    def __init__(self,
                 ...
                 input_c: int,
                 out_c: int,
                 expanded_ratio: int,  # 1 or 6 the first 1x1 conv expanded ratio
                 stride: int,          # 1 or 2
                 use_se: bool,         # True
                 drop_rate: float,
                 index: str,           # 1a, 2a, 2b, ...
...
class EfficientNet(nn.Module):
    def __init__(self,
                 ...
                 dropout_rate: float = 0.2, # before final fc layer
