...
parser.add_argument('--src_npy1', type=str, dest='src_npy1', default='')
parser.add_argument('--src_npy2', type=str, dest='src_npy2', default='')
parser.add_argument('--compare_folder', type=str, dest='compare_folder', default='./plot/scripts/old_f/')
parser.add_argument('--dest', type=str, dest='dest', default='')
parser.add_argument('--label1', type=str, dest='label1', default='npys')
parser.add_argument('--label2', type=str, dest='label2', default='npys')
...
x1, y1 = calc_cmc(args.src_npy1)
x2, y2 = calc_cmc(args.src_npy2)

xmin, xmax = plt.xlim()
ymin, ymax = plt.ylim()

lines = plt.plot(x1, y1, label='')
plt.setp(lines, 'color', predefined_line_color[0], 'linewidth', 4, 'label', args.label1)
lines = plt.plot(x2, y2, label='')
plt.setp(lines, 'color', predefined_line_color[1], 'linewidth', 4, 'label', args.label2)

for k, old_fi in enumerate(old_f):
    yi = read_cmc(join(args.compare_folder, old_fi))
    lines = plt.plot(x1, yi[:10], label='')
    plt.setp(lines, 'color', predefined_line_color[k + 2], 'linewidth', 4, 'label', old_fi)
...
plt.yticks(np.array([0.980, 0.984, 0.988, 0.992, 0.996, 1]), fontsize=16)
